...
import { CreditResultItem, getExcelDownloadUrl, getPdfDownloadUrl } from '@/lib/api/credit';
...
export default function DetailPanel({ item, simulationId, onClose }: DetailPanelProps) {
  ...
  const { tablasComparativas: tc, tablaAmortizacion: amort, plazoUsuario } = (() => {
    // We need plazoUsuario from the parent; we'll derive it from tablasComparativas indirectly.
    // The parent passes plazoMeses through item context — we use tablasComparativas.plazoUsuario.
    return {
      tablasComparativas: item.tablasComparativas,
      tablaAmortizacion: item.tablaAmortizacion,
      plazoUsuario: item.tablaAmortizacion.length, // actual plazo from amortization table length
    };
  })();

  const semaforo = SEMAFORO_COLORS[item.semaforo];

  // Determine which amortization rows to show
  ...
  // Is it a "gap" row (sentinel) — we'll splice in a gap indicator between first 12 and last 6
  ...
  return (
    ...
    >
      ...
      {/* Amortization table */}
      <div style={{ marginBottom: simulationId ? '28px' : '0' }}>
      ...
      {simulationId && (
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginTop: '4px' }}>
          ...
          <button
            onClick={() => window.open(getPdfDownloadUrl(simulationId), '_blank')}
            style={{
              background: 'transparent',
              border: '1px solid rgba(255,255,255,.15)',
              borderRadius: '8px',
